...
# - configure and start rstash-dist and scheduler
...
# of pot.
#
# After running it copies the rstash log file into
...
init()
{
	...
	TEST_TMPDIR=$(mktemp -d "/tmp/rstash_freebsd.XXXXXXX") || exit 1
	chmod g+r "$TEST_TMPDIR"
	export XDG_CONFIG_HOME="$TEST_TMPDIR/.config"
	mkdir -p "$XDG_CONFIG_HOME"
	export RSTASH_DIR="$TEST_TMPDIR/.cache"
	killall rstash 2>/dev/null || true
	killall rstash-dist 2>/dev/null || true
...
{
	echo "#### building rstash (cargo)"
	cd "$base"
	FAULT=0
	export RUSTFLAGS="-C debuginfo=0"
	cargo build --features "dist-client,dist-server" || FAULT=1
	echo "#### testing rstash (cargo)"
	cargo test --features "dist-client,dist-server" -- \
	  --test-threads 1 || FAULT=1
	unset RUSTFLAGS
	if [ "$FAULT" -eq 0 ]; then
		# save build time by avoiding "cargo install"
		cp -a target/debug/rstash target/debug/rstash-dist \
		  "$HOME/.cargo/bin/."
	fi
	if [ $FAULT -ne 0 ]; then return 1; fi
}

prepare_and_run_rstash_dist()
{
	echo "#### preparing rstash-dist"
	SECRET_KEY="$(rstash-dist auth generate-jwt-hs256-key)"
	CLIENT_AUTH_KEY="$(rstash-dist auth generate-jwt-hs256-key)"
	...
	EOF
	SERVER_TOKEN="$(rstash-dist auth generate-jwt-hs256-server-token \
	...
	# create rstash client config
	TC="$(rustup toolchain list | grep default | awk '{ print $1 }')"
	RUSTC_PATH="$HOME/.rustup/toolchains/$TC/bin/rustc"
	mkdir -p "$XDG_CONFIG_HOME/rstash"
	cat >"$XDG_CONFIG_HOME/rstash/config" <<-EOF
	[dist]
	scheduler_url = "http://127.0.0.1:10600"
	toolchain_cache_size = 5368709120
	cache_dir = "$HOME/.cache/rstash-dist-client"
	[dist.auth]
	type = "token"
	token = "$CLIENT_AUTH_KEY"
	[[dist.toolchains]]
	...
	rstash-dist scheduler --config "$TEST_TMPDIR"/scheduler.conf
}
...
prepare_pot()
{
	...
	sudo pot create -p rstash-template -N alias -i "lo0|127.0.0.2" \
	  -t single -b "$OS_VERSION"
	sudo pot set-cmd -p rstash-template -c /usr/bin/true
	sudo pot set-attr -p rstash-template -A no-rc-script -V YES
	sudo pot snapshot -p rstash-template
}

start_build_server()
{
	echo "#### starting build-server (as root)"
	RSTASH_DIST_LOG=debug RUST_LOG=info sudo \
	  "$HOME"/.cargo/bin/rstash-dist server \
...
start_rstash_server()
{
	echo "#### starting rstash-server"
	killall rstash 2>/dev/null || true
	RSTASH_ERROR_LOG="$TEST_TMPDIR"/rstash_log.txt RSTASH_LOG=info \
	  RUST_LOG=info rstash --start-server
	sleep 10
}
...
test_rstash_dist_01()
{
	...
	RUSTC_WRAPPER=rstash cargo build
	STATS="$(rstash -s)"
	...
	# on the first remote build (which will make rstash fall-back
	...
	if [ "$FAILED_DIST" -gt 1 ]; then
		...
		cat "$TEST_TMPDIR"/rstash_log.txt
		return 1
	fi
...
cleanup()
{
	...
	rstash --stop-server
	killall rstash
	killall rstash-dist && sleep 3
	sudo killall rstash-dist && sleep 3
	sudo killall -9 rstash-dist
	killall rstash
	cp "$TEST_TMPDIR/rstash_log.txt" "$base/rstash_log_$(date +%s).txt"
