...
pub static RSTASH_BIN: Lazy<PathBuf> = Lazy::new(|| assert_cmd::cargo::cargo_bin("rstash"));
...
pub struct RstashTest<'a> {
    /// Tempdir used for Rstash cache and cargo output. It is kept in the struct only to have the
    /// destructor run when RstashTest goes out of scope, but is never used otherwise.
    #[allow(dead_code)]
...
impl RstashTest<'_> {
    pub fn new(additional_envs: Option<&[(&'static str, std::ffi::OsString)]>) -> Result<Self> {
        ...
        Command::new(RSTASH_BIN.as_os_str())
            .arg("--start-server")
            .env("RSTASH_DIR", &cache_dir)
            .assert()
            .try_success()
        ...
        let mut env = vec![
            ...
            ("RUSTC_WRAPPER", RSTASH_BIN.as_os_str().to_owned()),
            // Explicitly disable incremental compilation because rstash is unable to cache it at
            // the time of writing.
    ...
    /// Show the statistics for rstash. This will be called at the end of a test and making this
...
impl Drop for RstashTest<'_> {
    fn drop(&mut self) {
        stop_rstash().expect("Stopping Rstash server failed");
    }
}

pub fn stop_rstash() -> Result<()> {
    ...
    Command::new(RSTASH_BIN.as_os_str())
        ...
        .stderr(Stdio::null())
        .status()
        .context("Failed to stop rstash server")?;
    Ok(())
}

pub fn cargo_clean(test_info: &RstashTest) -> Result<()> {
