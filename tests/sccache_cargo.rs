...
use helpers::{cargo_clean, stop_rstash, CARGO, CRATE_DIR};

use assert_cmd::prelude::*;
use fs_err as fs;
use helpers::{RstashTest, RSTASH_BIN};
...
fn test_rust_cargo_check() -> Result<()> {
    test_rust_cargo_cmd("check", RstashTest::new(None)?)
}

#[test]
#[serial]
fn test_rust_cargo_check_readonly() -> Result<()> {
    test_rust_cargo_cmd_readonly("check", RstashTest::new(None)?)
}

#[test]
#[serial]
fn test_rust_cargo_build() -> Result<()> {
    test_rust_cargo_cmd("build", RstashTest::new(None)?)
}

#[test]
#[serial]
fn test_rust_cargo_build_readonly() -> Result<()> {
    test_rust_cargo_cmd_readonly("build", RstashTest::new(None)?)
}
...
fn test_run_log_no_perm() -> Result<()> {
    trace!("rstash with log");
    stop_rstash()?;
    let mut cmd = Command::new(RSTASH_BIN.as_os_str());
    cmd.arg("gcc")
        .env("RSTASH_ERROR_LOG", "/no-perm.log") // Should not work
        .env("RSTASH_LOG", "debug");
...
fn test_run_log() -> Result<()> {
    ...
    let tempdir = tempfile::Builder::new()
        .prefix("rstash_test_rust_cargo")
        .tempdir()
        .context("Failed to create tempdir")?;
    let tmppath = tempdir.path().join("perm.log");
    let mut cmd = Command::new(RSTASH_BIN.as_os_str());
    cmd.arg("--start-server")
        .env("RSTASH_ERROR_LOG", &tmppath) // Should not work
        .env("RSTASH_LOG", "debug");
...
/// rstash, causes a rebuild and is correctly printed to stdout.
#[test]
#[serial]
fn test_rust_cargo_run_with_env_dep_parsing() -> Result<()> {
    test_rust_cargo_env_dep(RstashTest::new(None)?)
}
...
/// Test that building a simple Rust crate with cargo using rstash results in a cache hit
/// when built a second time and a cache miss, when the environment variable referenced via
/// env! is changed.
fn test_rust_cargo_cmd(cmd: &str, test_info: RstashTest) -> Result<()> {
...
fn restart_rstash(
    ...
    trace!("rstash --start-server");

    let mut cmd = Command::new(RSTASH_BIN.as_os_str());
    cmd.arg("--start-server");
    cmd.env("RSTASH_DIR", &cache_dir);
    ...
    cmd.assert()
        .try_success()
        .context("Failed to start rstash server")?;

    Ok(())
}

/// Test that building a simple Rust crate with cargo using rstash results in the following behaviors (for three different runs):
...
fn test_rust_cargo_cmd_readonly(cmd: &str, test_info: RstashTest) -> Result<()> {
    // `cargo clean` first, just to be sure there's no leftover build objects.
    cargo_clean(&test_info)?;
    ...
    restart_rstash(
        &test_info,
        Some(vec![("RSTASH_LOCAL_RW_MODE".into(), "READ_ONLY".into())]),
    )?;
    ...
    restart_rstash(
        &test_info,
        Some(vec![("RSTASH_LOCAL_RW_MODE".into(), "READ_WRITE".into())]),
    )?;
    Command::new(CARGO.as_os_str())
        .args([cmd, "--color=always"])
        .envs(test_info.env.iter().cloned())
...
fn test_rust_cargo_env_dep(test_info: RstashTest) -> Result<()> {
    cargo_clean(&test_info)?;
    // Now build the crate with cargo.
    Command::new(CARGO.as_os_str())
...
/// Test that building a simple Rust crate with cargo using rstash in read-only mode with an empty cache results in
...
fn test_rust_cargo_cmd_readonly_preemtive_block() -> Result<()> {
    let test_info = RstashTest::new(None)?;
    // `cargo clean` first, just to be sure there's no leftover build objects.
    cargo_clean(&test_info)?;

    let rstash_log = test_info.tempdir.path().join("rstash.log");
    ...
    restart_rstash(
        &test_info,
        Some(vec![
            ("RSTASH_LOCAL_RW_MODE".into(), "READ_ONLY".into()),
            ("RSTASH_LOG".into(), "trace".into()),
            (
                "RSTASH_ERROR_LOG".into(),
                rstash_log.to_str().unwrap().into(),
            ),
        ]),
    )?;
    ...
    let log_contents = fs::read_to_string(rstash_log)?;
