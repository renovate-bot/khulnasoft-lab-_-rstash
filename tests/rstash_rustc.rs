...
impl Drop for StopServer {
    fn drop(&mut self) {
        let _ = Command::cargo_bin("rstash")
            .unwrap()
            .arg("--stop-server")
            .ok();
    }
...
fn run_rstash(root: &Path, path: &Path) {
    let mut paths: OsString = path.into();
    paths.push(":");
    paths.push(var_os("PATH").unwrap());

    Command::cargo_bin("rstash")
        .unwrap()
        .current_dir(root)
        .env("PATH", paths)
        .env("RSTASH_DIR", root.join("rstash"))
        .arg("rustc")
        .arg("RUST_FILE.rs")
        .arg("--crate-name=rstash_rustc_tests")
        .arg("--crate-type=lib")
        .arg("--emit=link")
        .arg("--out-dir")
        .arg(root)
