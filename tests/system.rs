...
extern crate log;
use crate::harness::{
    get_stats, rstash_client_cfg, rstash_command, start_local_daemon, stop_local_daemon,
...
/* test case like this:
    ...
    mkdir o1 o2
    rstash g++ -c -g -gsplit-dwarf test.cc -o test1.o
    rstash g++ -c -g -gsplit-dwarf test.cc -o test1.o   --- > cache hit
    rstash g++ -c -g -gsplit-dwarf test.cc -o test2.o   --- > cache miss
    ...
    rstash_command()
        .args(
            [
                ...
                &vec_from!(OsString, "-DRSTASH_TEST_DEFINE")[..],
            ]
            .concat(),
        )
        .current_dir(tempdir)
...
fn run_rstash_command_tests(compiler: Compiler, tempdir: &Path, preprocessor_cache_mode: bool) {
...
fn test_nvcc_cuda_compiles(compiler: &Compiler, tempdir: &Path) {
    ...
    // Test to ensure #2299 doesn't regress (https://github.com/khulnasoft-lab/rstash/issues/2299)
...
fn run_rstash_nvcc_cuda_command_tests(compiler: Compiler, tempdir: &Path) {
...
fn run_rstash_clang_cuda_command_tests(compiler: Compiler, tempdir: &Path) {
...
fn run_rstash_hip_command_tests(compiler: Compiler, tempdir: &Path) {
    zero_stats();
...
fn test_rstash_command(preprocessor_cache_mode: bool) {
    let _ = env_logger::try_init();
    let tempdir = tempfile::Builder::new()
        .prefix("rstash_system_test")
        .tempdir()
        .unwrap();
    ...
    if compilers.is_empty() {
        ...
        // Ensure there's no existing rstash server running.
        stop_local_daemon();
        // Create the configurations
        let rstash_cfg = rstash_client_cfg(tempdir.path(), preprocessor_cache_mode);
        write_json_cfg(tempdir.path(), "rstash-cfg.json", &rstash_cfg);
        let rstash_cached_cfg_path = tempdir.path().join("rstash-cached-cfg");
        // Start a server.
        trace!("start server");
        start_local_daemon(
            &tempdir.path().join("rstash-cfg.json"),
            &rstash_cached_cfg_path,
        );
        for compiler in compilers {
            run_rstash_command_tests(compiler, tempdir.path(), preprocessor_cache_mode);
            zero_stats();
        }
        stop_local_daemon();
    }
...
#[serial]
#[cfg(any(unix, target_env = "msvc"))]
fn test_cuda_rstash_command(preprocessor_cache_mode: bool) {
    let _ = env_logger::try_init();
    let tempdir = tempfile::Builder::new()
        .prefix("rstash_system_test")
        .tempdir()
    ...
    if compilers.is_empty() {
        ...
        for compiler in compilers {
            match compiler.name {
                "nvcc" => run_rstash_nvcc_cuda_command_tests(compiler, tempdir.path()),
                "clang++" => run_rstash_clang_cuda_command_tests(compiler, tempdir.path()),
                _ => {}
            }
            zero_stats();
        }
...
#[serial]
#[cfg(any(unix, target_env = "msvc"))]
fn test_hip_rstash_command(preprocessor_cache_mode: bool) {
    let _ = env_logger::try_init();
    let tempdir = tempfile::Builder::new()
        .prefix("rstash_system_test")
        .tempdir()
    ...
    if let Some(compiler) = find_hip_compiler() {
        ...
        run_rstash_hip_command_tests(compiler, tempdir.path());
        zero_stats();
        stop_local_daemon();
    }
}
