...
// The configuration below is for the rstash-test tenant under aidanhs' auth0 account. There
...
fn config_with_dist_auth(
    tmpdir: &Path,
    auth_config: rstash::config::DistAuth,
...
fn rstash_command() -> Command {
    Command::new(assert_cmd::cargo::cargo_bin("rstash"))
}
...
fn test_auth() {
    // Make sure the client auth port isn't in use, as rstash will gracefully fall back
    let client_auth_port = rstash::dist::client_auth::VALID_PORTS[0];
    assert_eq!(
...
fn test_auth_with_config(dist_auth: rstash::config::DistAuth) {
    let conf_dir = tempfile::Builder::new()
        .prefix("rstash-test-conf")
        .tempdir()
        .unwrap();
    let rstash_config = config_with_dist_auth(conf_dir.path(), dist_auth);
    let rstash_config_path = conf_dir.path().join("rstash-config.json");
    fs::File::create(&rstash_config_path)
        .unwrap()
        .write_all(&serde_json::to_vec(&rstash_config).unwrap())
        .unwrap();
    let rstash_cached_config_path = conf_dir.path().join("rstash-cached-config");
    let envs = vec![
        ("RSTASH_LOG", "rstash=trace".into()),
        ("RSTASH_CONF", rstash_config_path.into_os_string()),
        (
            "RSTASH_CACHED_CONF",
            rstash_cached_config_path.clone().into_os_string(),
        ),
    ];

    println!("Starting rstash --dist-auth");
    let mut rstash_process = rstash_command()
        .arg("--dist-auth")
        .envs(envs)
        .stdin(Stdio::null())
        .spawn()
