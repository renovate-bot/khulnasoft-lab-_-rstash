...
pub fn find_rstash_binary() -> PathBuf {
    // Older versions of cargo put the test binary next to the rstash binary.
    ...
    dirs.iter()
        .map(|d| d.join("rstash").with_extension(env::consts::EXE_EXTENSION))
        .filter_map(|d| fs::metadata(&d).ok().map(|_| d))
        .next()
        .unwrap_or_else(|| {
            panic!(
            "Error: rstash binary not found, looked in `{:?}`. Do you need to run `cargo build`?",
            dirs
        )
        })
}
