...
use crate::server::{DistClientContainer, RstashServer, ServerMessage};
    ...
    T: Into<Option<ServerOptions>> + Send + 'static,
{
    ...
    let handle = thread::spawn(move || {
        ...
        let srv = RstashServer::new(0, runtime, client, dist_client, storage).unwrap();
        let mut srv: RstashServer<_, Arc<Mutex<MockCommandCreator>>> = srv;
...
fn test_server_stats() {
    ...
    // Include rstash ver (cli) to validate.
...
// https://github.com/khulnasoft-lab/rstash/issues/234
#[cfg(not(target_os = "macos"))]
fn test_server_port_in_use() {
    // Bind an arbitrary free port.
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let rstash = find_rstash_binary();
    let output = Command::new(rstash)
        .arg("--start-server")
        .env(
            "RSTASH_SERVER_PORT",
            listener.local_addr().unwrap().port().to_string(),
        )
        .env_remove("RSTASH_SERVER_UDS")
        .output()
        .unwrap();
