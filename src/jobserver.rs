...
// The execution model of rstash is that on the first run it spawns a server
...
// When executing the rust compiler through the rstash server, that jobserver
...
// when the rstash server is spawned, but that means that in some cases, the
// cargo or make process can't terminate until the rstash server terminates
// after its idle timeout (which also never happens if RSTASH_IDLE_TIMEOUT=0).
// Also, if the rstash server ends up shared between multiple runs of
// cargo/make, then which jobserver is used doesn't make sense anymore.
// Ideally, the rstash client would give a handle to the jobserver it has
// access to, so that the rust compiler would "just" use the jobserver it
// would have used if it had run without rstash, but that adds some extra
