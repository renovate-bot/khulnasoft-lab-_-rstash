    ...
    impl<S: dist::SchedulerIncoming + 'static> Scheduler<S> {
        ...
        pub fn start(self) -> Result<Infallible> {
            ...
            let server = rouille::Server::new(public_addr, move |request| {
                ...
                trace!(target: "rstash_http", "Req {} ({}): {:?}", req_id, request.remote_addr(), request);
                let response = (|| router!(request,
                    ...
                    (POST) (/api/v1/scheduler/heartbeat_server) => {
                        ...
                        trace!(target: "rstash_heartbeat", "Req {}: heartbeat_server: {:?}", req_id, heartbeat_server);
                 ...
                 trace!(target: "rstash_http", "Res {}: {:?}", req_id, response);
                response
            }).map_err(|e| anyhow!(format!("Failed to start http server for rstash scheduler: {}", e)))?;
    ...
    impl<S: dist::ServerIncoming + 'static> Server<S> {
        ...
        pub fn start(self) -> Result<Infallible> {
            ...
            thread::spawn(move || {
                ...
                loop {
                    trace!(target: "rstash_heartbeat", "Performing heartbeat");
                    match bincode_req(
                        client
                        ...
                        Ok(HeartbeatServerResult { is_new }) => {
                            trace!(target: "rstash_heartbeat", "Heartbeat success is_new={}", is_new);
                        ...
                        Err(e) => {
                            error!(target: "rstash_heartbeat", "Failed to send heartbeat to server: {}", e);
            ...
            let server = rouille::Server::new_ssl(bind_address, move |request| {
                ...
                response
            }, cert_pem, privkey_pem).map_err(|e| anyhow!(format!("Failed to start http server for rstash server: {}", e)))?;
