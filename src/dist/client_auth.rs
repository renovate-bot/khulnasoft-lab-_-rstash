...
mod code_grant_pkce {
    ...
    pub fn code_to_token(
        ...
        if expires_at - Instant::now() < MIN_TOKEN_VALIDITY {
            ...
            eprintln!(
                "rstash: Token retrieved expires in under {}",
                MIN_TOKEN_VALIDITY_WARNING
            );
        }
        Ok(token)
    ...
    E: std::fmt::Debug,
{
    let body = format!("{:?}", e);
    eprintln!(
        "rstash: Error during a request to {} on the client auth web server\n{}",
        uri, body
    );
    let len = body.len();
...
pub fn get_token_oauth2_code_grant_pkce(
    ...
    println!(
        "rstash: Please visit http://localhost:{} in your browser",
        port
    );
