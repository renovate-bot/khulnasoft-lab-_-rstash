...
/// The default rstash server port.
...
fn get_addr() -> crate::net::SocketAddr {
    #[cfg(unix)]
    if let Ok(addr) = env::var("RSTASH_SERVER_UDS") {
        if let Ok(uds) = crate::net::SocketAddr::parse_uds(&addr) {
            return uds;
        }
    }
    let port = env::var("RSTASH_SERVER_PORT")
        .ok()
...
fn ignore_all_server_io_errors() -> bool {
    match env::var("RSTASH_IGNORE_SERVER_IO_ERROR") {
...
fn run_server_process(startup_timeout: Option<Duration>) -> Result<ServerStartup> {
    ...
    let tempdir = tempfile::Builder::new().prefix("rstash").tempdir()?;
    ...
    let _child = process::Command::new(&exe_path)
        .current_dir(workdir)
        .env("RSTASH_START_SERVER", "1")
        .env("RSTASH_STARTUP_NOTIFY", &socket_path)
...
fn create_error_log() -> Result<File> {
    ...
    let name = match env::var("RSTASH_ERROR_LOG") {
        Ok(filename) if !filename.is_empty() => filename,
        _ => {
            bail!("Cannot read variable 'RSTASH_ERROR_LOG'");
        }
    };
...
/// If `RSTASH_ERROR_LOG` is set, redirect stderr to it.
...
fn run_server_process(startup_timeout: Option<Duration>) -> Result<ServerStartup> {
    ...
    let mut envp = {
        let mut v = vec![];
        let extra_vars = vec![
            (OsString::from("RSTASH_START_SERVER"), OsString::from("1")),
            (
                OsString::from("RSTASH_STARTUP_NOTIFY"),
...
/// Attempt to connect to an rstash server listening on `addr`, or start one if no server is running.
            ...
            match run_server_process(startup_timeout)? {
                ServerStartup::Ok { addr: actual_addr } => {
                    if addr.to_string() != actual_addr {
                        ...
                        bail!(
                            "rstash: Listening on address {actual_addr} instead of {addr}"
                        );
                    }
                }
...
fn handle_compile_finished(
    ...
    if let Some(ret) = response.retcode {
        ...
        println!("rstash: Compiler killed by signal {}", signal);
        Ok(-2)
    } else {
        println!("rstash: Missing compiler exit status!");
        Ok(-3)
    }
}
    ...
    T: CommandCreatorSync,
{
                ...
                Err(e) => {
                    match e.downcast_ref::<io::Error>() {
                        Some(io_e) if io_e.kind() == io::ErrorKind::UnexpectedEof => {
                            eprintln!(
                                "rstash: warning: The server looks like it shut down \
                        ...
                        _ => {
                            ...
                            if ignore_all_server_io_errors() {
                                eprintln!(
                                    "rstash: warning: error reading compile response from server \
    ...
    Ok(status.code().unwrap_or_else(|| {
        if let Some(sig) = status_signal(status) {
            println!("rstash: Compile terminated by signal {}", sig);
        }
        // Arbitrary.
        2
    }))
...
/// Send a `Compile` request to the rstash server `conn`, and handle the response.
///
...
pub fn run_command(cmd: Command) -> Result<i32> {
    ...
    match cmd {
        ...
        Command::InternalStartServer => {
            ...
            if env::var("RSTASH_ERROR_LOG").is_ok() {
        ...
        Command::StopServer => {
            ...
            println!("Stopping rstash server...");
            ...
            incr_env_strs
                ...
                .for_each(|incr_str| match env::var(incr_str) {
                    Ok(incr_val) if incr_val == "1" => {
                        println!(
                            "rstash: incremental compilation is prohibited: Unset {} to continue.",
                            incr_str
                        );
                        std::process::exit(1);
                    }
