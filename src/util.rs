...
///
/// If the command returns a non-successful exit status, an error of `RstashError::ProcessError`
...
pub fn daemonize() -> Result<()> {
    ...
    match env::var("RSTASH_NO_DAEMON") {
        Ok(ref val) if val == "1" => {}
        _ => {
    ...
    unsafe {
        match env::var("RSTASH_ALLOW_CORE_DUMPS") {
            Ok(ref val) if val == "1" => {
...
/// # TODO
///
/// We should refactor rstash current model to make sure that we only have
/// one tokio runtime and keep reqwest alive inside it.
///
/// ---
///
/// More details could be found at https://github.com/khulnasoft-lab/rstash/pull/1563
