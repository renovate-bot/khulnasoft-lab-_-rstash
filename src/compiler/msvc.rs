...
msvc_args!(static ARGS: [ArgInfo<ArgData>; _] = [
    ...
    msvc_take_arg!("Fp", PathBuf, Concatenated, TooHardPath), // allows users to specify the name for a PCH (when using /Yu or /Yc), PCHs are not supported in rstash.
    msvc_take_arg!("Fr", PathBuf, Concatenated, TooHardPath),
...
pub fn parse_arguments(
    ...
    for arg in it {
        ...
        match arg.get_data() {
            ...
            // to share the same PDB file, which is not supported by rstash. So either
            // -Fd was passed with a pdb that is not shared and rstash is going to
    ...
    T: RunCommand,
{
    ...
    if is_clang {
        ...
        // Read for more info: https://github.com/khulnasoft-lab/rstash/issues/1725
        // And here: https://github.com/khulnasoft-lab/rstash/issues/2250
        cmd.arg("/WX-");
    }
...
fn generate_compile_commands(
    ...
    let dist_command = (|| {
        ...
        // case we're safe as it only applies to link time, which rstash avoids).
...
mod test {
    ...
    fn test_compile_simple() {
        ...
        let service = server::RstashService::mock_with_storage(storage, runtime.handle().clone());
