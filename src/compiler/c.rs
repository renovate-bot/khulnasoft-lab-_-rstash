    ...
    fn parse_arguments(
        ...
        match self.compiler.parse_arguments(arguments, cwd, env_vars) {
            CompilerArguments::Ok(mut args) => {
                // Handle RSTASH_EXTRAFILES
                for (k, v) in env_vars.iter() {
                    if k.as_os_str() == OsStr::new("RSTASH_EXTRAFILES") {
...
fn process_preprocessed_file(
    ...
    while start < total_len.saturating_sub(7) {
            ...
            .is_some()
        {
            ...
            // hash is too hard for rstash, so just bail out.
...
/// - the new include file is hashed by rstash
...
#[cfg(feature = "dist-client")]
const PREPROCESSING_SKIPPED_COMPILE_POISON: &[u8] = b"([{RSTASH -*-* INVALID_C_CPP_CODE([{\"";
...
static CACHED_ENV_VARS: Lazy<HashSet<&'static OsStr>> = Lazy::new(|| {
    [
        // RSTASH_C_CUSTOM_CACHE_BUSTER has no particular meaning behind it,
