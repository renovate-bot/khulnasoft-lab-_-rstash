    ...
    S: SearchableArgInfo<ArgData>,
{
    ...
    let dont_cache_multiarch = env::var("RSTASH_CACHE_MULTIARCH").is_err();
    ...
    for arg in args_iter {
        ...
        // mess. See https://github.com/khulnasoft-lab/rstash/issues/150#issuecomment-318586953
        match arg {
            ...
            Some(Arch(arch)) => {
                match seen_arch {
                    Some(s) if &s != arch && dont_cache_multiarch => {
                        cannot_cache!(
                            "multiple different -arch, and RSTASH_CACHE_MULTIARCH not set"
                        )
                    }
                    _ => {}
                };
...
mod test {
    ...
    fn test_preprocess_cmd_rewrites_archs() {
        with_var("RSTASH_CACHE_MULTIARCH", Some("1"), || {
    ...
    fn test_parse_arguments_multiarch_cache_disabled() {
        with_var_unset("RSTASH_CACHE_MULTIARCH", || {
            assert_eq!(
                CompilerArguments::CannotCache(
                    "multiple different -arch, and RSTASH_CACHE_MULTIARCH not set",
                    None
