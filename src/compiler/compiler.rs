...
pub async fn write_temp_file(
    ...
    pool.spawn_blocking(move || {
        let dir = tempfile::Builder::new().prefix("rstash").tempdir()?;
    ...
    T: CommandCreatorSync,
{
    ...
    let maybe_rustc_executable = if is_rustc_like(executable) {
        ...
        // If not, detect the scenario where cargo is configured to wrap rustc with something other than rstash.
        // This happens when rstash is used as a `RUSTC_WRAPPER` and another tool is used as a
...
LLVM version: 6.0",
    ...
    fn test_compiler_get_cached_or_compile(preprocessor_cache_mode: bool) {
        ...
        let service = server::RstashService::mock_with_storage(storage.clone(), pool.clone());
    ...
    fn test_compiler_get_cached_or_compile_dist(preprocessor_cache_mode: bool) {
        ...
        let service = server::RstashService::mock_with_dist_client(
            dist_client.clone(),
            storage.clone(),
            pool.clone(),
        );
