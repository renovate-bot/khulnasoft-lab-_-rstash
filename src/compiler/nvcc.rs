            ...
            CompilerArguments::Ok(mut parsed_args) => {
                match parsed_args.compilation_flag.to_str() {
                    ...
                    Some(flag) => {
                        ...
                        //  $ rstash nvcc x.cu -o x.bin
                        //  $ rstash nvcc x.cu -o x.cu.o -c
                        //  $ rstash nvcc x.cu -o x.ptx -ptx
                        //  $ rstash nvcc x.cu -o x.cubin -cubin
                        //
...
pub fn generate_compile_commands(
    ...
    arguments.extend(vec![
        ...
        // but we run the host compiler in `cwd` (the dir from which rstash was
...
impl CompileCommandImpl for NvccCompileCommand {
    ...
    async fn execute<T>(
        &self,
        service: &server::RstashService<T>,
        creator: &T,
    ) -> Result<process::Output>
    where
        T: CommandCreatorSync,
    ...
    T: CommandCreatorSync,
{
    ...
    // rstash was invoked. Arguments will be relative to the cwd, except
        ...
        if let (env_vars, cacheable, Some(group)) = match exe.file_stem().and_then(|s| s.to_str()) {
            ...
            _ => {
                ...
                if args.contains(&preprocessor_flag) {
                    ...
                    // as non-cacheable. This ensures `rstash nvcc ...` _always_ decomposes the
                        ...
                        env_vars
                            ...
                            .chain(
                                [
                                    ...
                                    // The RstashService API doesn't have a great way to specify this
                                    // case, so we set a special envvar here that it can read when the
                                    // compilation is finished.
                                    ("__RSTASH_THIS_IS_A_CUDA_COMPILATION__".into(), "".into()),
                                ]
                                .iter(),
                            )
                            .cloned()
                    ...
                    Ok(compiler) => match compiler.parse_arguments(&args, cwd, env_vars) {
                        ...
                        CompilerArguments::Ok(hasher) => {
                            srvc.start_compile_task(
                                ...
                                env_vars
                                    .iter()
                                    .chain([("RSTASH_DIRECT".into(), "false".into())].iter())
                                    .cloned()
                                    .collect::<Vec<_>>(),
                            )
                            .await
