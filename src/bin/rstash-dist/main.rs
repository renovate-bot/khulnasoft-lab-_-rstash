...
use rstash::util::daemonize;
use rstash::util::BASE64_URL_SAFE_ENGINE;
...
fn main() {
    ...
    let command = match cmdline::try_parse_from(env::args()) {
        ...
        Err(e) => match e.downcast::<clap::error::Error>() {
            ...
            Err(some_other_err) => {
                println!("rstash-dist: {some_other_err}");
                for source_err in some_other_err.chain().skip(1) {
                    println!("rstash-dist: caused by: {source_err}");
                }
                std::process::exit(1);
            }
        },
    ...
    std::process::exit(match run(command) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("rstash-dist: error: {}", e);

            for e in e.chain().skip(1) {
                eprintln!("rstash-dist: caused by: {}", e);
            }
            2
        }
    });
    ...
    match command {
        ...
        Command::Server(server_config::Config {
            ...
            let server = Server::new(builder, &cache_dir, toolchain_cache_size)
                .context("Failed to create rstash server instance")?;
            ...
            .context("Failed to create rstash HTTP server instance")?;
            http_server.start()?;
            unreachable!();
        }
    }
...
fn init_logging() {
    if env::var(rstash::LOGGING_ENV).is_ok() {
        match env_logger::Builder::from_env(rstash::LOGGING_ENV).try_init() {
            Ok(_) => (),
