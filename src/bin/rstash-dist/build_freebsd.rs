...
use rstash::lru_disk_cache::Error as LruError;
...
impl PotBuilder {
    ...
    pub fn new(
        ...
        let ret = Self {
            ...
            max_cleanup_thread_count: rstash::util::num_cpus() * 3,
        };
        ret.cleanup()?;
        Ok(ret)
    }
    ...
    fn cleanup(&self) -> Result<()> {
        ...
        let mut to_remove = Command::new(&self.pot_cmd)
            ...
            .context("Failed to force delete container")?
            .split('\n')
            .filter(|a| a.starts_with("rstash-builder-") || a.starts_with("rstash-image-"))
            .map(|s| s.to_string())
            .collect::<Vec<String>>();
        to_remove.sort();
    ...
    fn make_image(
        ...
        let imagename = format!("rstash-image-{}", &tc.archive_id);
    ...
    fn start_container(
        ...
        let cid = format!("rstash-builder-{}", Uuid::new_v4());
