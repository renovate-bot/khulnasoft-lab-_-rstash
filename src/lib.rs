...
/// VERSION is the pkg version of rstash.
///
...
/// logging for rstash, and rstash-dist.
pub const LOGGING_ENV: &str = "RSTASH_LOG";
...
pub fn main() {
    ...
    let command = match cmdline::try_parse() {
        ...
        Err(e) => match e.downcast::<clap::error::Error>() {
            ...
            Err(some_other_err) => {
                println!("rstash: {some_other_err}");
                for source in some_other_err.chain().skip(1) {
                    println!("rstash: caused by: {source}");
                }
                std::process::exit(1);
            }
        },
    ...
    std::process::exit(match commands::run_command(command) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("rstash: error: {}", e);
            for e in e.chain().skip(1) {
                eprintln!("rstash: caused by: {}", e);
            }
            2
        }
    });
