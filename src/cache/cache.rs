...
impl CacheWrite {
        ...
        T: Read,
    {
        ...
        let compression_level = std::env::var("RSTASH_CACHE_ZSTD_LEVEL")
            .ok()
...
impl Storage for opendal::Operator {
    ...
    async fn check(&self) -> Result<CacheMode> {
        ...
        match self.read(path).await {
            ...
            // Tricky Part.
            //
            // We tolerate rate limited here to make rstash keep running.
...
pub fn storage_from_config(
    ...
    if let Some(cache_type) = &config.cache {
        match cache_type {
            ...
            // we only want to use rstash with a local cache (no remote storage)
            _ => {}
        }
    }
