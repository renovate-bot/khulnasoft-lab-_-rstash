...
// Unfortunately this means that nothing else can use the rstash cache dir as
// this top level directory is used directly to store rstash cached objects...
...
fn config_from_env() -> Result<EnvConfig> {
    // ======= AWS =======
    let s3 = if let Ok(bucket) = env::var("RSTASH_BUCKET") {
        let region = env::var("RSTASH_REGION").ok();
        let no_credentials = bool_from_env_var("RSTASH_S3_NO_CREDENTIALS")?.unwrap_or(false);
        let use_ssl = bool_from_env_var("RSTASH_S3_USE_SSL")?;
        let server_side_encryption = bool_from_env_var("RSTASH_S3_SERVER_SIDE_ENCRYPTION")?;
        let endpoint = env::var("RSTASH_ENDPOINT").ok();
        let key_prefix = key_prefix_from_env_var("RSTASH_S3_KEY_PREFIX");
        let enable_virtual_host_style = bool_from_env_var("RSTASH_S3_ENABLE_VIRTUAL_HOST_STYLE")?;
    ...
    {
        bail!("If setting S3 credentials, RSTASH_S3_NO_CREDENTIALS must not be set.");
    }

    // ======= redis =======
    let redis = match (
        env::var("RSTASH_REDIS").ok(),
        env::var("RSTASH_REDIS_ENDPOINT").ok(),
        env::var("RSTASH_REDIS_CLUSTER_ENDPOINTS").ok(),
    ) {
        (None, None, None) => None,
        (url, endpoint, cluster_endpoints) => {
            let db = number_from_env_var("RSTASH_REDIS_DB")
                .transpose()?
                .unwrap_or(DEFAULT_REDIS_DB);

            let username = env::var("RSTASH_REDIS_USERNAME").ok();
            let password = env::var("RSTASH_REDIS_PASSWORD").ok();

            let ttl = number_from_env_var("RSTASH_REDIS_EXPIRATION")
                .or_else(|| number_from_env_var("RSTASH_REDIS_TTL"))
                .transpose()?
                .unwrap_or(DEFAULT_REDIS_CACHE_TTL);

            let key_prefix = key_prefix_from_env_var("RSTASH_REDIS_KEY_PREFIX");
    ...
    let memcached = if let Ok(url) =
        env::var("RSTASH_MEMCACHED").or_else(|_| env::var("RSTASH_MEMCACHED_ENDPOINT"))
    {
        let username = env::var("RSTASH_MEMCACHED_USERNAME").ok();
        let password = env::var("RSTASH_MEMCACHED_PASSWORD").ok();

        let expiration = number_from_env_var("RSTASH_MEMCACHED_EXPIRATION")
            .transpose()?
            .unwrap_or(DEFAULT_MEMCACHED_CACHE_EXPIRATION);

        let key_prefix = key_prefix_from_env_var("RSTASH_MEMCACHED_KEY_PREFIX");
    ...
    if env::var_os("RSTASH_MEMCACHED").is_some()
        && env::var_os("RSTASH_MEMCACHED_ENDPOINT").is_some()
    {
        bail!("You mustn't set both RSTASH_MEMCACHED and RSTASH_MEMCACHED_ENDPOINT. Please, use only RSTASH_MEMCACHED_ENDPOINT.");
    }

    // ======= GCP/GCS =======
    if (env::var("RSTASH_GCS_CREDENTIALS_URL").is_ok()
        || env::var("RSTASH_GCS_OAUTH_URL").is_ok()
        || env::var("RSTASH_GCS_KEY_PATH").is_ok())
        && env::var("RSTASH_GCS_BUCKET").is_err()
    {
        bail!(
            "If setting GCS credentials, RSTASH_GCS_BUCKET and an auth mechanism need to be set."
        );
    }

    let gcs = env::var("RSTASH_GCS_BUCKET").ok().map(|bucket| {
        let key_prefix = key_prefix_from_env_var("RSTASH_GCS_KEY_PREFIX");

        if env::var("RSTASH_GCS_OAUTH_URL").is_ok() {
            eprintln!("RSTASH_GCS_OAUTH_URL has been deprecated");
            eprintln!("if you intend to use vm metadata for auth, please set correct service account instead");
        }

        let credential_url = env::var("RSTASH_GCS_CREDENTIALS_URL").ok();

        let cred_path = env::var("RSTASH_GCS_KEY_PATH").ok();
        let service_account = env::var("RSTASH_GCS_SERVICE_ACCOUNT").ok();

        let rw_mode = match env::var("RSTASH_GCS_RW_MODE").as_ref().map(String::as_str) {
            ...
            Ok(_) => {
                warn!("Invalid RSTASH_GCS_RW_MODE -- defaulting to READ_ONLY.");
                CacheModeConfig::ReadOnly
            }
            _ => {
                warn!("No RSTASH_GCS_RW_MODE specified -- defaulting to READ_ONLY.");
                CacheModeConfig::ReadOnly
            }
        };
    ...
    // ======= GHA =======
    let gha = if let Ok(version) = env::var("RSTASH_GHA_VERSION") {
        // If RSTASH_GHA_VERSION has been set, we don't need to check
        // RSTASH_GHA_ENABLED's value anymore.
        Some(GHACacheConfig {
            enabled: true,
            version,
        })
    } else if bool_from_env_var("RSTASH_GHA_ENABLED")?.unwrap_or(false) {
        // If only RSTASH_GHA_ENABLED has been set to the true value, enable with
        // default version.
        Some(GHACacheConfig {
            enabled: true,
            version: "".to_string(),
    ...
    let azure = if let (Ok(connection_string), Ok(container)) = (
        env::var("RSTASH_AZURE_CONNECTION_STRING"),
        env::var("RSTASH_AZURE_BLOB_CONTAINER"),
    ) {
        let key_prefix = key_prefix_from_env_var("RSTASH_AZURE_KEY_PREFIX");
    ...
    // ======= WebDAV =======
    let webdav = if let Ok(endpoint) = env::var("RSTASH_WEBDAV_ENDPOINT") {
        let key_prefix = key_prefix_from_env_var("RSTASH_WEBDAV_KEY_PREFIX");
        let username = env::var("RSTASH_WEBDAV_USERNAME").ok();
        let password = env::var("RSTASH_WEBDAV_PASSWORD").ok();
        let token = env::var("RSTASH_WEBDAV_TOKEN").ok();
    ...
    // ======= OSS =======
    let oss = if let Ok(bucket) = env::var("RSTASH_OSS_BUCKET") {
        let endpoint = env::var("RSTASH_OSS_ENDPOINT").ok();
        let key_prefix = key_prefix_from_env_var("RSTASH_OSS_KEY_PREFIX");

        let no_credentials = bool_from_env_var("RSTASH_OSS_NO_CREDENTIALS")?.unwrap_or(false);
    ...
    {
        bail!("If setting OSS credentials, RSTASH_OSS_NO_CREDENTIALS must not be set.");
    }

    // ======= Local =======
    let disk_dir = env::var_os("RSTASH_DIR").map(PathBuf::from);
    let disk_sz = env::var("RSTASH_CACHE_SIZE")
        .ok()
        .and_then(|v| parse_size(&v));

    let mut preprocessor_mode_config = PreprocessorCacheModeConfig::activated();
    let preprocessor_mode_overridden = if let Some(value) = bool_from_env_var("RSTASH_DIRECT")? {
    ...
    let (disk_rw_mode, disk_rw_mode_overridden) = match env::var("RSTASH_LOCAL_RW_MODE")
        .as_ref()
        .map(String::as_str)
    {
        ...
        Ok(_) => {
            warn!("Invalid RSTASH_LOCAL_RW_MODE -- defaulting to READ_WRITE.");
...
impl Config {
    pub fn load() -> Result<Self> {
        ...
        let file_conf_path = config_file("RSTASH_CONF", "config");
...
impl CachedConfig {
    ...
    fn file_config_path() -> PathBuf {
        config_file("RSTASH_CACHED_CONF", "cached-config")
    }
...
pub mod server {
    ...
    const DEFAULT_POT_CLONE_FROM: &str = "rstash-template";
...
fn test_s3_no_credentials_conflict() {
    env::set_var("RSTASH_S3_NO_CREDENTIALS", "true");
    env::set_var("RSTASH_BUCKET", "my-bucket");
    env::set_var("AWS_ACCESS_KEY_ID", "aws-access-key-id");
    env::set_var("AWS_SECRET_ACCESS_KEY", "aws-secret-access-key");

    let cfg = config_from_env();

    env::remove_var("RSTASH_S3_NO_CREDENTIALS");
    env::remove_var("RSTASH_BUCKET");
    env::remove_var("AWS_ACCESS_KEY_ID");
    env::remove_var("AWS_SECRET_ACCESS_KEY");

    let error = cfg.unwrap_err();
    assert_eq!(
        "If setting S3 credentials, RSTASH_S3_NO_CREDENTIALS must not be set.",
        error.to_string()
    );
}
...
fn test_s3_no_credentials_invalid() {
    env::set_var("RSTASH_S3_NO_CREDENTIALS", "yes");
    env::set_var("RSTASH_BUCKET", "my-bucket");
    ...
    let error = cfg.unwrap_err();
    assert_eq!(
        "RSTASH_S3_NO_CREDENTIALS must be 'true', 'on', '1', 'false', 'off' or '0'.",
        error.to_string()
    );
}
...
fn test_s3_no_credentials_valid_false() {
    env::set_var("RSTASH_S3_NO_CREDENTIALS", "false");
    env::set_var("RSTASH_BUCKET", "my-bucket");
...
fn test_gcs_service_account() {
    env::set_var("RSTASH_GCS_BUCKET", "my-bucket");
    env::set_var("RSTASH_GCS_SERVICE_ACCOUNT", "my@example.com");
    env::set_var("RSTASH_GCS_RW_MODE", "READ_WRITE");

    let cfg = config_from_env();

    env::remove_var("RSTASH_GCS_BUCKET");
    env::remove_var("RSTASH_GCS_SERVICE_ACCOUNT");
    env::remove_var("RSTASH_GCS_RW_MODE");
    ...
    assert_eq!(
        ...
        FileConfig {
            cache: CacheConfigs {
                ...
                disk: Some(DiskCacheConfig {
                    dir: PathBuf::from("/tmp/.cache/rstash"),
            ...
            dist: DistConfig {
                ...
                cache_dir: PathBuf::from("/home/user/.cache/rstash-dist-client"),
                toolchains: vec![],
                toolchain_cache_size: 5368709120,
                rewrite_includes_only: false,
            },
