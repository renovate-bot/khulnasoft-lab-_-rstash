...
// limitations under the License.RSTASH_MAX_FRAME_LENGTH
...
pub struct DistClientConfig {
    // Reusable items tied to an RstashServer instance
...
impl DistClientContainer {
    ...
    fn new(config: &Config, _: &tokio::runtime::Handle) -> Self {
        if config.dist.scheduler_url.is_some() {
            warn!("Scheduler address configured but dist feature disabled, disabling distributed rstash")
        }
        Self {}
    }
...
impl DistClientContainer {
    ...
    async fn create_state(config: DistClientConfig) -> DistClientState {
        ...
        match config.scheduler_url {
            Some(ref addr) => {
                let url = addr.to_url();
                info!("Enabling distributed rstash to {}", url);
                ...
                let auth_token = try_or_fail_with_message!(auth_token
                    .context("could not load client auth token, run |rstash --dist-auth|"));
            ...
            None => {
                info!("No scheduler address configured, disabling distributed rstash");
                DistClientState::Disabled
            }
        }
    }
...
/// Start an rstash server, listening on `addr`.
///
...
pub fn start_server(config: &Config, addr: &crate::net::SocketAddr) -> Result<()> {
    ...
    let notify = env::var_os("RSTASH_STARTUP_NOTIFY");
    ...
    let res = (|| -> io::Result<_> {
        match addr {
            crate::net::SocketAddr::Net(addr) => {
                ...
                let srv =
                    RstashServer::<_>::with_listener(l, runtime, client, dist_client, storage);
                Ok((
...
pub struct RstashServer<A: crate::net::Acceptor, C: CommandCreatorSync = ProcessCommandCreator> {
    runtime: Runtime,
    listener: A,
    rx: mpsc::Receiver<ServerMessage>,
    timeout: Duration,
    service: RstashService<C>,
    wait: WaitUntilZero,
}

impl<C: CommandCreatorSync> RstashServer<tokio::net::TcpListener, C> {
    pub fn new(
        port: u16,
        runtime: Runtime,
        client: Client,
...
impl<A: crate::net::Acceptor, C: CommandCreatorSync> RstashServer<A, C> {
    pub fn with_listener(
        ...
        let pool = runtime.handle().clone();
        let service = RstashService::new(dist_client, storage, &client, pool, tx, info);
        ...
        A::Socket: 'static,
    {
        ...
        // instances of `RstashService` have been dropped.
        //
...
/// Service implementation for rstash
#[derive(Clone)]
pub struct RstashService<C>
where
    C: Send,
{
    /// Server statistics.
...
type RstashRequest = Message<Request, Body<()>>;
type RstashResponse = Message<Response, Pin<Box<dyn Future<Output = Result<Response>> + Send>>>;
...
impl<C> Service<RstashRequest> for Arc<RstashService<C>>
where
    C: CommandCreatorSync + Send + Sync + 'static,
{
    type Response = RstashResponse;
    type Error = Error;
    type Future = Pin<Box<dyn Future<Output = Result<Self::Response>> + Send + 'static>>;

    fn call(&mut self, req: RstashRequest) -> Self::Future {
        trace!("handle_client");
    ...
    C: CommandCreatorSync + Clone + Send + Sync + 'static,
{
        ...
        T: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        if let Ok(max_frame_length_str) = env::var("RSTASH_MAX_FRAME_LENGTH") {
            if let Ok(max_frame_length) = max_frame_length_str.parse::<usize>() {
                builder.max_frame_length(max_frame_length);
            } else {
                warn!("Content of RSTASH_MAX_FRAME_LENGTH is not a valid number, using default");
            }
        }
        let io = builder.new_framed(socket);

        let (sink, stream) = RstashTransport {
    ...
    async fn handle_compile(&self, compile: Compile) -> Result<RstashResponse> {
        let exe = compile.exe;
    ...
    pub async fn start_compile_task(
        ...
        let force_recache = env_vars.iter().any(|(k, _v)| k == "RSTASH_RECACHE");
        let force_no_cache = env_vars.iter().any(|(k, _v)| k == "RSTASH_NO_CACHE");
        ...
        let (kind, lang) = {
            ...
            if env_vars
                .iter()
                .any(|(k, _)| k == "__RSTASH_THIS_IS_A_CUDA_COMPILATION__")
            {
                (
                        ...
                        match err.downcast::<ProcessError>() {
                            ...
                            Err(err) => match err.downcast::<HttpClientError>() {
                                ...
                                Err(err) => {
                                    ...
                                    let mut error = "rstash: encountered fatal error\n".to_string();
                                    let _ = writeln!(error, "rstash: error: {}", err);
                                    for e in err.chain() {
                                        error!("[{:?}] \t{}", out_pretty, e);
                                        let _ = writeln!(error, "rstash: caused by: {}", e);
                                    }
...
///   below.
struct RstashTransport<I: AsyncRead + AsyncWrite + Unpin> {
    inner: Framed<
...
impl<I: AsyncRead + AsyncWrite + Unpin> Stream for RstashTransport<I> {
...
impl<I: AsyncRead + AsyncWrite + Unpin> Sink<Frame<Response, Response>> for RstashTransport<I> {
    type Error = Error;
