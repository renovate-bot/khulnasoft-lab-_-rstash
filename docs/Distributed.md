...
Distributed rstash consists of three parts:

 - the client, an rstash binary that wishes to perform a compilation on
   remote machines
 - the scheduler (`rstash-dist` binary), responsible for deciding where
   a compilation job should run
 - the server (`rstash-dist` binary), responsible for actually executing
   a build
...
The HTTP implementation of rstash has the following API, where all HTTP body content is encoded using [`bincode`](http://docs.rs/bincode):
...
Create a scheduler key with `rstash-dist auth generate-jwt-hs256-key` (which will
...
```
rstash-dist auth generate-jwt-hs256-server-token \
...
If a client is malicious, they can cause a DoS of distributed rstash servers or
...
The client should then run `rstash --dist-auth` and follow the instructions to retrieve
...
Securing communication with the scheduler is the responsibility of the rstash cluster
...
Use the `--config` argument to pass the path to its configuration file to `rstash-dist`.
...
Until these binaries [are included in releases](https://github.com/khulnasoft-lab/rstash/issues/393) I've put together a Docker container that can be used to easily build a release binary:
```
docker run -ti --rm -v $PWD:/rstash luser/rstash-musl-build:0.1 /bin/bash -c "cd /rstash; cargo build --release --target x86_64-unknown-linux-musl --features=dist-server && strip target/x86_64-unknown-linux-musl/release/rstash-dist && cd target/x86_64-unknown-linux-musl/release/ && tar czf rstash-dist.tar.gz rstash-dist"
```
